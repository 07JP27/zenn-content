...
## 増分モード（Incremental mode）
増分モードは、一般的にイメージしやすい挙動です。リソースグループには存在するが、テンプレートに定義されていないリソースは**変更せず、そのまま残します**。 名前の通り、追加・変更の増分のみを行うモードです。
リソースグループに存在せずテンプレートに定義されたリソースは、リソースグループに追加されます。リソースグループに存在しており、テンプレートで定義されているものは**プロパティが再適用**されます(更新)。
