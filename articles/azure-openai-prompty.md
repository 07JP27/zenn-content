---
title: "PromptyでプロンプトセントリックなAIアプリ開発をしよう"
emoji: "🤖"
...
コード内に定数として定義したり、データベースに格納したりすることが多かったのではないでしょうか。
...
また、この仕組みを利用して、プロンプトの変数が複雑になったり、いくつかのサンプルデータを簡単に差し替えてテストしたい場合にも有用です。
たとえば冒頭のサンプルは次のようにJSONファイルに切り出せます。
