...
たとえばvocabularyTest.prompt.mdには以下のような内容が記載されています。
```markdown
---
name: vocabularyTest
description: 英語の単語に関するテストを実施する。
---

vocabularyTestは、ユーザーの英語の単語力を評価するためのテストです。  
ユーザーに対して英単語のテストを10問出題してください。

現在のユーザーの英語レベルはREADME.mdに記載されています。初期問題を出題する際には、ユーザーの現在のCEFRスコアを考慮して、適切な難易度の問題を考えてください。
learninghistory/Vocabulary.mdには過去に学習した単語のリストが記録されています。
このリストを参照して学習曲線を意識しながら新出と既出の単語をバランスよく混合して出題してください。
出題は1問ずつ"AskUserQuestion"ツールを使用して行い、全ての問題に回答してから、正解を発表してください。
正解発表後に正誤を集計してユーザーにフィードバックしてください。
評価後にはCEFRスコアの更新とlearninghistory/Vocabulary.md（新出は追加・既出は更新）も忘れずに行ってください。
```
