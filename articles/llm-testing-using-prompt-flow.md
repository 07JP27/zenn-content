...
LLMを使用したLLMの評価は「[Is ChatGPT a Good NLG Evaluator? A Preliminary Study](https://arxiv.org/pdf/2303.04048.pdf)」という論文が発表されるなど、注目され始めています。**評価に使用するLLMを都度変更してしまうと評価基準がずれることになるので、同じモデルを使用して評価を行うことが重要です**。
